...
void *generate(void *args){
    struct calcul *argument = args;
    while (1)
    {
        pthread_mutex_lock(&argument->mutex);
        size_t localY = *(argument->y);
        if(localY == argument->height){
            pthread_mutex_unlock(&argument->mutex);
			pthread_exit(0);
        }
        *(argument->y) = *(argument->y)+1;
        pthread_mutex_unlock(&argument->mutex);

        for (unsigned int x = 0; x < argument->width; x++)
        {
            calcul(argument->image, argument->out_image, argument->width, argument->height, x, localY);
        }
    }
}
...
int main(void)
{
    ...
    int N = 4;
    size_t y = 0;
    //Création du mutex pour accéder à x
    pthread_mutex_t mutex;
	pthread_mutex_init(&mutex, NULL);
    //Initialisation de la structure qui sera passée en argument de chaque thread.
    struct calcul argument;
    argument.image = image;
    argument.out_image = out_image;
    ...
    for(int i = 0; i<N;i++){
		pthread_create(&thread[i],NULL,generate,&argument);
	}
