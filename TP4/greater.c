...
int main(int argc, char **argv) {
    ...
    for(int i = 0; i<nombreIterations;i++){
        ...
        //Conserve le i
        numero = i;
        //fork et conserve l'id pour vérifier qui est le parent et pour pouvoir donner le pid du fil au processus n+1
        idfork = fork();
        //Si on est le parent
        if(idfork == 0){
        ...
        //Pour chque itération qui n'est pas la première, lit le meilleur numéro du process n-1 et le ompare avec le sien
        if(i>0){
